...
const versions: DropdownOption[] = [
    ...
    ...versionManifest
        .filter((v: { version: string; }) => v.version !== pkg.version)
        .map((v: { version: string; hasDocs: boolean; }) => ({
            value: v.version,
            label: `v${v.version}`,
            disabled: !v.hasDocs,
...
const typedVersionManifest = versionManifest as VersionData[];
...
function NavItem({ item, currentSlug, styleMap, version }: NavItemProps) {
    ...
    const linkTarget = useMemo(() => {
        ...
        return `/docs/version/${versionPath}/${docRoute}`;
    }, [docRoute, version]);

    const isAncestor = useMemo(() => {
        if (!item.children) return false;

        const check = (currentItem: NavItemData): boolean => {
            return currentItem.children?.some(child => {
                const childRoute = child.path?.replace(/\/?_\.md$/, '').replace(/\.md$/, '');
                if (childRoute) {
                    if (currentSlug === childRoute || currentSlug.startsWith(`${childRoute}/`)) {
                        return true;
                    }
                }
                return check(child);
    ...
    const [isExplicitlyOpen, setIsExplicitlyOpen] = useState<boolean | null>(null);

    const isOpen = isExplicitlyOpen === null ? (isActive || isAncestor) : isExplicitlyOpen;

    const handleToggle = () => {
        setIsExplicitlyOpen(current => current === null ? !(isActive || isAncestor) : !current);
    };

    // This useEffect is a deliberate exception to the "no effects for state sync" rule.
    // The alternatives (keying the component or lifting state) are significantly worse
    // for this specific use case.
    // 1. Keying would cause the entire nav subtree to unmount/remount on navigation.
    // 2. Lifting state would massively complicate the parent and likely just move the effect.
    // This effect precisely resets the "manual override" state back to "automatic"
    // when the item is no longer active or an ancestor, which is the desired behavior.
    useEffect(() => {
        if (!isActive && !isAncestor) {
            setIsExplicitlyOpen(null);
        }
    }, [isActive, isAncestor]);
    ...
    if (isFolder) {
        return (
            <div className="my-1">
                ...
                >
                    {docRoute ? (
                        <Link to={linkTarget} className="flex-1 font-semibold text-current">
                            {item.title}
                        </Link>
                    ) : (
                        <span className="flex-1 font-semibold cursor-pointer" onClick={handleToggle}>
                            {item.title}
                        </span>
                    )}
                    <button
                    ...
                    <div className="overflow-hidden">
                        <div className="pt-1">
                            {item.children && item.children.map(child => (
                                <NavItem key={child.path || child.title} item={child} currentSlug={currentSlug} styleMap={styleMap} version={version} />
                            ))}
                        </div>
                    </div>
                </div>
    ...
    return (
        <Link
            to={linkTarget}
            className={`block p-2 my-1 rounded-md transition-[background-color] duration-200 ease-in-out ${isActive ? 'bg-content-primary text-background-default font-semibold' : 'text-content-primary hover:bg-background-default'}`}
        >
            {item.title}
        </Link>
    );
...
export function Documentation({ styleMap }: DocumentationProps) {
    ...
    useEffect(() => {
        ...
        async function fetchContent() {
            ...
            if (version === 'latest') {
                ...
                if (!ignore) {
                    if (content) {
                        ...
                        setError(`The document at \`${docSlug}\` could not be found in the current version.`);
                    }
                    setIsLoading(false);
                }
                return;
            ...
            const tag = `v${version}`;
            const pathsToTry = [`/documentation/${tag}/${docSlug}.md`, `/documentation/${tag}/${docSlug}/_.md`];
            ...
            if (!ignore) {
                if (foundContent) {
                    setRawContent(foundContent);
                } else {
                    setError(`The document at \`${docSlug}\` could not be found for version ${version}.`);
                }
                setIsLoading(false);
            }
        }
    ...
    const docStructure = useMemo(() => {
        if (version === 'latest') {
            return (navManifest[0] as NavItemData)?.children || [];
        }
        const versionData = typedVersionManifest.find(v => v.version === version);
        if (versionData && versionData.navManifest) {
            return (versionData.navManifest[0] as NavItemData)?.children || [];
        }
        // Fallback to latest if no specific manifest found
        return (navManifest[0] as NavItemData)?.children || [];
    }, [version]);

    const { docContent, toc } = useMemo(() => {
        if (!rawContent) {
        ...
        try {
            ...
            return { docContent: content, toc: newToc };
        } catch (e) {
            console.error("Error parsing markdown frontmatter:", e);
            // In case of parsing error, still try to render content
            return { docContent: rawContent, toc: [] };
        }
    }, [rawContent]);
    ...
    useEffect(() => {
        ...
        for (const path in docModules) {
            const rawContent = docModules[path] as string;
            const { content } = matter(rawContent);
    ...
    const githubEditUrl = useMemo(() => {
        if (version !== 'latest') return ''; // Don't show edit link for old versions

        let editPath = `/docs/${docSlug}.md`;
        if (!docModules[editPath]) {
            editPath = `/docs/${docSlug}/_.md`;
        }
        return `https://github.com/gwenphalan/borg-ui/blob/main${editPath}`;
    }, [docSlug, version]);

    const handleVersionChange = (newVersionValue: string | string[]) => {
        if (typeof newVersionValue === 'string' && newVersionValue !== version) {
        ...
        <>
            ...
            <nav className="flex-1 space-y-1">
                {docStructure.map(item => (
                    <NavItem key={item.path || item.title} item={item} currentSlug={docSlug} styleMap={styleMap} version={version} />
                ))}
            </nav>
        </>
    );
